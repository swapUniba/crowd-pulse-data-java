 ...
 */
@SuppressWarnings({"unchecked", "deprecation"}) public class HonestDAO<T, K> implements DAO<T, K> {
    //CHECKSTYLE:OFF
    /**
    ...
    protected void initDAO(final Datastore ds) {
        ...
        initType(((Class<T>) ((ParameterizedType) getClass().getGenericSuperclass())
    ...
    @SuppressWarnings("unchecked") public List<K> findIds() {
        return (List<K>) keysToIds(ds.find(entityClazz).asKeyList());
    }

    @SuppressWarnings("unchecked") public List<K> findIds(final Query<T> q) {
        return (List<K>) keysToIds(q.asKeyList());
    }

    @SuppressWarnings("unchecked") public List<K> findIds(final String key, final Object value) {
